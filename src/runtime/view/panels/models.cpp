// models.cpp — the fused cross-provider model picker (^/).
//
...
Element models_panel(const Model& m) {
    ...
    cfg.title = picker->assign_slot
        ? std::string{" Smart Mode \xc2\xb7 pick "}
          + std::string{smart::role_display_name(*picker->assign_slot)}
          + " model "
